...
const Gender = () => {
    ...
    const renderGenderItem = (gender: GenderProps) => (
        <TouchableOpacity style={[styles.genderItemContainer, gender.value === formik.values.gender && styles.genderItemSelectedContainer]} key={gender.value} onPress={() => formik.setFieldValue('gender', gender.value)}>
            <View>
                <Text style={styles.genderText}>{gender.label}</Text>
                <Text style={styles.subText}>{gender.subText}</Text>
            </View>
            <View style={styles.genderIcon}>
                <Iconify icon={gender.value === 'male' ? 'famicons:male' : 'famicons:female'} size={60} color={gender.value === formik.values.gender ? colors.PRIMARY : colors.ACCENT_FOREGROUND} />
            </View>
        </TouchableOpacity>
    )
...
const styles = StyleSheet.create({
    ...SignupStyles, genderItemContainer: {
        flex: 1,
        height: 220,
        padding: 16,
        borderRadius: 24,

    ...
    return (
        ...
        >
            ...
            <View style={styles.weightAssessmentContainer}>
                    ...
                    <View style={styles.weightUnitsContainer}>
                        {
                            units.map((unit, index) => (
                                <TouchableOpacity key={index} style={{ ...styles.weightUnitItem, backgroundColor: unit === selectedUnit ? colors.LIGHT_BG : "transparent" }} onPress={() => setSelectedUnit(unit)}>
                ...
                <View style={{ gap: 8 }}>
                    <ButtonUI label='Continue' backgroundColor={"#4B3425"} iconOnRight children={<Iconify icon="solar:arrow-right-bold" color={colors.WHITE} size={20} style={{ position: "absolute", right: 24 }} />} style={{ paddingHorizontal: 24, paddingVertical: 16, borderRadius: 1000 }} labelStyle={{ marginRight: 24, fontSize: 14, fontFamily: Font.FontBold }} onPress={() => router.push("/(health-assessment)/blood-type")} />
                    <Text style={styles.footerText}><Link href="/(health-assessment)/blood-type" style={{ color: colors.ORANGE }}>Skip for now</Link></Text>

import { DefaultSectionT, Dimensions, SectionList, SectionListData, StyleSheet, TouchableOpacity, View } from 'react-native'
    ...
    const renderHeader = () => (
        <View style={styles.headerContainer}>
            <View style={styles.avatarContainer}>
                <Avatar size={100} backgroundColor={colors.PRIMARY} source={{ uri: "https://a0.anyrgb.com/pngimg/226/368/user-profile-profile-avatar-heroes-user-male-silhouette-monochrome-black-icons.png" }} />
                <TouchableOpacity style={styles.avatarPlusBtn}><Iconify icon="solar:pen-outline" color={colors.WHITE} size={12} /></TouchableOpacity>
            ...
            <View style={styles.emailContainer}>
                ...
                <TouchableOpacity><Iconify icon="solar:pen-outline" color={colors.PRIMARY} size={16} /></TouchableOpacity>
            </View>
        </View>
    )
        ...
        return (
            <TouchableOpacity style={styles.itemContainer} onPress={item.onPress}>
                <View style={{ gap: 7, flexDirection: "row", alignItems: "center" }}>
                    <Iconify icon={item.icon ?? "solar:user-id-outline"} color={item.color ?? colors.BLACK} size={16} />
                    <Text style={{ ...styles.itemTitle, color: item.color ?? colors.BLACK }}>{item.title}</Text>
                </View>
                <View>
                    {
                        item.showTrailingAccessory ? item.trailingAccessory ? item.trailingAccessory : <Iconify icon="solar:alt-arrow-right-outline" color={item.color ?? colors.BLACK} size={16} /> : null
                    }
                </View>
            </TouchableOpacity>
        )
    ...
    return (
        <MainContent isPadded style={{ paddingTop: 0 }}>
            <SectionList
                ...
                } renderSectionHeader={({ section }) => <Text style={{ ...styles.emailText, fontSize: 11, marginBottom: 7 }}>{section.title}</Text>}
